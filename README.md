# Chamelaion Go API Library

<!-- x-release-please-start-version -->

<a href="https://pkg.go.dev/github.com/stainless-sdks/chamelaion-go"><img src="https://pkg.go.dev/badge/github.com/stainless-sdks/chamelaion-go.svg" alt="Go Reference"></a>
...
```go
import (
	"github.com/stainless-sdks/chamelaion-go" // imported as chamelaion
)
```
...
```sh
go get -u 'github.com/stainless-sdks/chamelaion-go@v0.0.1'
```
...
The chamelaion library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`json:"...,required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `chamelaion.String(string)`, `chamelaion.Int(int64)`, etc.
...
```go
p := chamelaion.ExampleParams{
	ID:   "id_xxx",                 // required property
	Name: chamelaion.String("..."), // optional property

	Point: chamelaion.Point{
		X: 0,                 // required field will serialize as 0
		Y: chamelaion.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: chamelaion.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[chamelaion.FooParams](12)
```
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/chamelaion-go/option).
...
`*chamelaion.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `chamelaion.File(reader io.Reader, filename string, contentType string)`
which can be used to wrap any `io.Reader` with the appropriate file name and content type.

```go
// A file from the file system
file, err := os.Open("/path/to/file")
chamelaion.LipsyncGenerateWithMediaParams{
	Audio: file,
...
chamelaion.LipsyncGenerateWithMediaParams{
	Audio: chamelaion.File(strings.NewReader(`{"hello": "foo"}`), "file.go", "application/json"),
	Video: io.Reader(bytes.NewBuffer([]byte("Example data"))),
}
```
...
params := FooNewParams{
    ...
    Data: FooNewParamsData{
        FirstName: chamelaion.String("John"),
    },
}
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/chamelaion-go/issues) with questions, bugs, or suggestions.
